...
이 구성은 AMD64 또는 ARM64 Linux 서버의 Docker Engine에서 Ubuntu 22.04 기반 SKOJ 웹 애플리케이션과 MariaDB, Redis, Celery, Bridge, DMOJ Judge를 함께 실행합니다. Docker가 호스트 아키텍처에 맞는 이미지를 자동으로 선택합니다. 운영 `compose.yaml`은 이미지에 포함된 소스를 사용하고, 로컬 개발에서 자동으로 병합되는 `compose.override.yaml`만 호스트 소스를 mount합니다. 기본 Blue Web은 `127.0.0.1:8001`, Green Web은 `127.0.0.1:8002`에 공개됩니다.
...
```sh
docker compose --env-file .env.docker up -d --no-deps --force-recreate web-blue
docker compose --env-file .env.docker exec web-blue python manage.py shell -c \
...
flowchart TB
    ...
    N -->|127.0.0.1:8001 또는 :8002| W[Django Web]
    W -->|제출 저장| DB[(MariaDB)]
...
```sh
docker compose --env-file .env.docker build init web-blue celery-blue bridge
docker compose --env-file .env.docker --profile tools run --rm init
docker compose --env-file .env.docker up -d db redis web-blue celery-blue bridge judge judge-02
```

처음에는 Ubuntu 패키지, Python 라이브러리, DMOJ 런타임을 내려받으므로 시간이 걸릴 수 있습니다.

`init`은 명시적으로 실행하는 유지보수 도구입니다. Web 시작과 연결하지 않으므로 운영 Blue/Green 배포 중 마이그레이션이 암묵적으로 실행되지 않습니다. MariaDB와 Redis가 정상 상태인 상태에서 다음 작업을 수행합니다.
...
`init` 종료 코드 0을 확인한 뒤 Web, Celery, Bridge를 시작합니다. 초기화는 반복 실행해도 안전하며 같은 이름의 Judge가 있으면 중복 생성하지 않고 각 인증 키를 현재 `.env.docker` 값으로 갱신합니다.
...
```sh
docker compose --env-file .env.docker exec web-blue python manage.py createsuperuser
```

기존 관리자 비밀번호를 변경하려면 다음 명령을 사용합니다.

```sh
docker compose --env-file .env.docker exec web-blue python manage.py changepassword admin
```

브라우저에서 <http://127.0.0.1:8001> 또는 <http://127.0.0.1:8001/admin>을 엽니다.

서비스 상태와 로그는 다음 명령으로 확인합니다.

```sh
docker compose --env-file .env.docker ps
docker compose --env-file .env.docker logs -f web-blue bridge judge judge-02
```
...
다시 시작할 때는 `docker compose --env-file .env.docker up -d`를 실행합니다. 스키마 변경이 있다면 먼저 `--profile tools run --rm init`을 명시적으로 실행해야 합니다. MariaDB 데이터, 정적 파일, 애플리케이션 로그, 문제 데이터는 각각 호스트의 `./data/mariadb`, `./data/static`, `./logs`, `./problems`에 유지됩니다. Redis 데이터는 Docker named volume에 유지됩니다.
...
- `web-blue`, `web-green`: Gunicorn worker 5개로 실행하며 각각 `127.0.0.1:8001`, `127.0.0.1:8002`에만 공개
- `db`: MariaDB 10.11, Docker 내부 네트워크 전용, 데이터는 `./data/mariadb`에 저장
- 정적 파일: `init` 또는 배포 스크립트에서 `./data/static/releases/<RELEASE_ID>`에 수집
- `redis`: 캐시 및 Celery 브로커, Docker 내부 네트워크 전용
- `init`: 명시적으로 실행하는 마이그레이션·초기화 도구
- `celery-blue`, `celery-green`: Web 릴리스와 같은 이미지로 롤링 교체하는 비동기 작업 처리기
...
개발 override에서는 `./site`가 컨테이너에 mount됩니다. 운영에서는 mount하지 않으며 Git SHA 이미지와 `deploy.sh` 또는 `restart.sh`로만 코드를 교체합니다. Django 계열 서비스의 파일 로그는 `./logs`에 저장됩니다. Docker 전용 설정은 추적 중인 `settings.example.py`를 기본값으로 사용하고 `docker/django/local_settings.py`에서 Linux 경로와 서비스 주소를 적용합니다.
