...
from judge.models import AlgorithmGuide, DifficultyCluster, GuideCompletion, Language, ProblemGroup, \
    ProfileGamification, ChallengeAttempt, ChallengeExam, Submission, Tier
...
class ChallengeViewTestCase(TestCase):
    def setUp(self):
        self.user = get_user_model().objects.create_user(username='challenge-view-user')
        self.group = ProblemGroup.objects.create(name='challenge-stack', full_name='스택')
        ...
        self.regular = create_problem(code='challenge-practice', group=self.group, is_public=True, points=10)
        self.second_regular = create_problem(code='challenge-practice2', group=self.group, is_public=True, points=10)
        self.exam = ChallengeExam.objects.create(title='실버 승급 시험', source_tier=Tier.BRONZE)
        self.first = create_problem(
            code='challenge-first', name='비밀 승급 문제 하나', group=self.group,
            is_public=False, challenge_exam=self.exam, challenge_order=1, points=10,
        )
        self.second = create_problem(
            code='challenge-second', name='비밀 승급 문제 둘', group=self.group,
            is_public=True, challenge_exam=self.exam, challenge_order=2, points=10,
        )
        self.url = reverse('gamification_challenge')
    ...
    def test_login_is_required(self):
        self.assertEqual(self.url, '/challenge/')
        self.client.logout()
        response = self.client.get(self.url)
        self.assertRedirects(response, '/accounts/login/?next=/challenge/', fetch_redirect_response=False)
        self.assertNotContains(self.client.get(reverse('home')), '비밀 승급 문제')

    def test_old_url_redirects_to_challenge_without_exposing_locked_problems(self):
        for authenticated in (True, False):
            with self.subTest(authenticated=authenticated):
                if not authenticated:
                    self.client.logout()
                response = self.client.get('/promotion/?source=bookmark')
                self.assertRedirects(response, '/challenge/?source=bookmark', status_code=301,
                                     fetch_redirect_response=False)
                self.assertNotContains(response, self.first.name, status_code=301)
    ...
    def test_navigation_is_after_problems_and_active(self):
        ...
        self.assertContains(response, 'href="/challenge/" class="active" aria-current="page"')
    ...
    def test_locked_page_shows_conditions_but_never_exam_names_or_links(self):
        ...
        self.assertFalse(response.context_data['challenge_eligible'])
        ...
        self.assertFalse(ChallengeAttempt.objects.filter(profile=self.user.profile).exists())
    ...
    def test_only_distinct_full_public_regular_solves_count(self):
        ...
        self.assertFalse(response.context_data['challenge_attempt'])
        self.assertNotContains(response, self.first.name)

    def test_every_active_group_is_required(self):
        group = ProblemGroup.objects.create(name='challenge-queue', full_name='큐')
        cluster = DifficultyCluster.objects.create(
            tier=Tier.BRONZE, problem_group=group, required_solve_count=1, ranking_weight=1,
        )
        self.unlock()
        response = self.client.get(self.url)
        self.assertEqual(response.context_data['challenge_completed_groups'], 1)
        self.assertFalse(response.context_data['challenge_attempt'])
        cluster.is_active = False
        cluster.save(update_fields=('is_active',))
        self.assertIsNotNone(self.client.get(self.url).context_data['challenge_attempt'])
    ...
    def test_eligible_page_unlocks_and_links_ordered_problems_idempotently(self):
        ...
        self.assertFalse(ChallengeAttempt.objects.filter(profile=self.user.profile).exists())
        response = self.client.get(self.url)
        self.assertTrue(response.context_data['challenge_eligible'])
        ...
        self.assertEqual(ChallengeAttempt.objects.filter(profile=self.user.profile).count(), 1)

    def test_another_users_unlock_does_not_expose_my_exam(self):
        other = get_user_model().objects.create_user(username='other-challenge-user')
        for problem in (self.regular, self.second_regular):
            self.solve(problem, user=other)
        self.assertTrue(ChallengeAttempt.objects.filter(profile=other.profile).exists())
        response = self.client.get(self.url)
        self.assertNotContains(response, self.first.name)
    ...
    def test_pre_unlock_solves_do_not_show_as_completed(self):
        ...
        self.unlock()
        response = self.client.get(self.url)
        self.assertEqual(response.context_data['challenge_solved_count'], 0)
        self.solve(self.first, points=5)
        self.assertEqual(self.client.get(self.url).context_data['challenge_solved_count'], 0)
        self.solve(self.first)
        response = self.client.get(self.url)
        self.assertEqual(response.context_data['challenge_solved_count'], 1)
    ...
    def test_completing_exam_updates_tier_and_next_requirements(self):
        ...
        self.assertEqual(response.context_data['challenge_target_tier'], '골드')
        self.assertIsNotNone(ChallengeAttempt.objects.get(profile=self.user.profile).completed_at)
        self.assertNotContains(response, self.first.name)
    ...
    def test_eligible_with_empty_exam_shows_preparing_message(self):
        self.exam.problems.update(challenge_exam=None)
        self.unlock()
        response = self.client.get(self.url)
        self.assertTrue(response.context_data['challenge_eligible'])
        self.assertContains(response, '승급전을 준비 중입니다')
    ...
    def test_master_has_no_next_tier_or_locked_exam(self):
        ...
        self.assertIsNone(response.context_data['challenge_target_tier'])
    ...
    def test_home_learning_cards_are_off_by_default_without_losing_data(self):
        ...
        self.assertNotContains(response, 'id="tier-panel-challenge"')
        ...
        self.assertContains(response, reverse('gamification_challenge'))
    ...
    def test_home_still_syncs_challenge_when_cards_are_hidden(self):
        Submission.objects.bulk_create([
            Submission(user=self.user.profile, problem=problem, language=Language.get_python3(),
                       status='D', result='AC', points=problem.points)
            for problem in (self.regular, self.second_regular)
        ...
        response = self.client.get(reverse('home'))
        self.assertTrue(ChallengeAttempt.objects.filter(profile=self.user.profile).exists())
        self.assertNotContains(response, self.first.name)

    def test_challenge_page_does_not_load_home_only_context(self):
