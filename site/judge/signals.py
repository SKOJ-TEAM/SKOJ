...
def submission_delete(sender, instance, **kwargs):
    ...
    from judge.models import ProfileGamification, ChallengeAttemptProblem
    affects_attempt = ChallengeAttemptProblem.objects.filter(
    ...
    if affects_cluster or instance.problem.challenge_exam_id or affects_attempt:
...
def update_gamification_from_submission(sender, instance, **kwargs):
    if not instance.is_graded:
        return
    from judge.models import ChallengeAttemptProblem
    affects_attempt = ChallengeAttemptProblem.objects.filter(
        attempt__profile=instance.user, attempt__completed_at__isnull=True, problem=instance.problem,
    ).exists()
    affects_cluster = instance.problem.group.difficulty_clusters.filter(is_active=True).exists()
    if not (affects_cluster or instance.problem.challenge_exam_id or affects_attempt):
        return
    from judge.gamification import sync_profile_gamification
    sync_profile_gamification(instance.user)
